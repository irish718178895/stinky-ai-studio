...
import { CHECKPOINT, COMFY_URL, PROJECT_FILES_DIR } from "../config.js";
...
function makeWorkflow({ prompt, negativePrompt, width, height, steps, cfg, seed }) {
  return {
    ...
    "7": { class_type: "SaveImage", inputs: { filename_prefix: "Stinky_AI_Studio", images: ["6", 0] } }
  };
}
...
async function waitForResult(promptId, timeoutMs = 10 * 60 * 1000) {
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    if (job?.status?.status_str === "error") throw new Error(`ComfyUI generation failed: ${JSON.stringify(job.status.messages || [])}`);
    const images = [];
    for (const output of Object.values(job?.outputs || {})) for (const image of output.images || []) images.push(image);
...
export async function generateForProject(projects, project, settings, sourceImageId = null) {
  ...
  const seed = Number.isSafeInteger(suppliedSeed) && suppliedSeed > 0 ? suppliedSeed : crypto.randomInt(1, 2_147_483_647);
  if (!prompt) throw new Error("Prompt is required.");
  const queued = await comfyFetch("/prompt", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: makeWorkflow({ prompt, negativePrompt, width, height, steps, cfg, seed }), client_id: crypto.randomUUID() }) });
  const queueResult = await queued.json();
  if (!queueResult.prompt_id) throw new Error(`ComfyUI did not return a prompt_id: ${JSON.stringify(queueResult)}`);
  ...
  for (const output of outputs) {
    ...
    const record = { id: imageId, url, originalFilename: output.filename, prompt, negativePrompt, seed, width, height, steps, cfg, checkpoint: CHECKPOINT, sampler: "euler", scheduler: "normal", promptId: queueResult.prompt_id, sourceImageId, createdAt: new Date().toISOString() };
