...
async function loadProviders() {
  const response = await jsonFetch("/api/providers");

  providerCatalog.defaults = response.defaults || {};
...
function showProjectDialog(project = null) {
  ...
  byId("projectDialogEyebrow").textContent =
    project ? "EDIT PROJECT" : "NEW PROJECT";

  byId("projectDialogTitle").textContent =
    project ? "Edit project" : "Create a project";

  byId("saveProjectButton").textContent =
  ...
  populateProviderSelect(
    ...
    "image",
    project?.providers?.image
  );

  renderProviderSettings(
    "image",
    byId("projectImageProvider").value,
  ...
  populateProviderSelect(
    ...
    "story",
    project?.providers?.story
  );

  renderProviderSettings(
    "story",
    byId("projectStoryProvider").value,
    project?.providerSettings?.[
      byId("projectStoryProvider").value
    ] || {}
  );
  ...
  populateProviderSelect(
    ...
    "voice",
    project?.providers?.voice
  );

  renderProviderSettings(
    "voice",
    byId("projectVoiceProvider").value,
    project?.providerSettings?.[
      byId("projectVoiceProvider").value
    ] || {}
  );
  ...
  renderProviderSettings(
    "render",
    byId("projectRenderProvider").value,
    project?.providerSettings?.[
      byId("projectRenderProvider").value
    ] || {}
  );
...
function renderProviderSettings(type, providerId, values = {}) {
  const container = byId(
    `project${type.charAt(0).toUpperCase() + type.slice(1)}ProviderSettings`
  );

  container.innerHTML = "";

  const provider = (providerCatalog.providers[type] || [])
    .find(item => item.id === providerId);

  if (!provider || !Array.isArray(provider.settingsSchema)) {
    return;
  }

  for (const setting of provider.settingsSchema) {
    ...
    if (setting.type === "select") {
      ...
      // Preserve a previously saved value even when the provider no longer
      // advertises it, so editing a project does not silently discard it.
      ...
      if (
        ...
        !normalizedOptions.includes(normalizedCurrentValue)
      ) {
        const legacyOption = document.createElement("option");
        legacyOption.value = normalizedCurrentValue;
        legacyOption.textContent = `${normalizedCurrentValue} (saved)`;
        control.append(legacyOption);
      }
      ...
      for (const optionDefinition of options) {
        ...
        if (
          ...
          option.value = String(optionDefinition);
          option.textContent = String(optionDefinition);
        }
      ...
      if (!control.options.length) {
        ...
        unavailableOption.value = normalizedCurrentValue;
        unavailableOption.textContent = normalizedCurrentValue || "No options available";
        control.append(unavailableOption);
      }

      control.value = normalizedCurrentValue;
    } else {
    ...
    control.dataset.provider = provider.id;
    control.dataset.setting = setting.key;

    if (setting.placeholder !== undefined && "placeholder" in control) {
...
function collectProviderSettings() {
  ...
  document
    .querySelectorAll("[data-provider][data-setting]")
    .forEach(input => {
      ...
      providerSettings[providerId][settingKey] = value;
    });
...
async function saveProject(event) {
  ...
  description: byId("projectDesc").value,

providers: {
  image: byId("projectImageProvider").value,
  story: byId("projectStoryProvider").value,
  voice: byId("projectVoiceProvider").value,
  render: byId("projectRenderProvider").value
},
...
[
  ...
  byId(
    `project${type.charAt(0).toUpperCase() + type.slice(1)}Provider`
...
function populateProviderSelect(selectId, type, selectedId) {
  const select = byId(selectId);

  select.innerHTML = "";

  const providers = providerCatalog.providers[type] || [];
...
if (provider.supports?.gpu) {
    badges.push("GPU");
}

if (provider.supports?.cpu) {
    badges.push("CPU");
}

if (provider.supports?.remote) {
    badges.push("Remote");
}
...
option.textContent =
    badges.length
        ? `${provider.name} (${badges.join(", ")})`
        : provider.name;
    option.selected =
      provider.id === (selectedId || providerCatalog.defaults[type]);
